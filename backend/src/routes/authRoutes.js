...
const authGuard = (req, res, next) => {
  try {
    ...
    req.user = dec; // <-- para conservar roles en /refresh si quieres
    next();
  } catch {
...
// Firma token con payload completo (incluye roles)
const signFullToken = ({ rut, email, roles }) => {
  const rolesNorm = (roles || []).map(r => String(r || '').toLowerCase()).filter(Boolean);
  const role = rolesNorm[0] || null;
  const payload = {
    sub: rut,       // compat
    id: rut,        // tu middleware usa req.user.id
    rut,            // explícito
    email: email || null,
    role,
    roles: rolesNorm.length ? rolesNorm : (role ? [role] : []),
  };
...
router.post('/login', async (req, res) => {
  try {
    ...
    if (isBcryptHash(ua.password_hash)) {
      ...
      ok = ua.password_hash === password; // DEV
    }
    if (!ok) return res.status(401).json({ success: false, message: 'Credenciales inválidas' });

    // Obtener email y roles reales
    const rutClean = normalizeRut(ua.rut_persona);
    ...
    const p = persRows?.[0] || { rut: ua.rut_persona, email: null };
    ...
    // Token con roles
    const token = signFullToken({ rut: ua.rut_persona, email: p?.email, roles });
...
router.post('/register', async (req, res) => {
  ...
  try {
        ...
        `INSERT INTO personas
         ...
         VALUES
          (:rut, :rutFmt, :nombres, :apPat, NULL,
           ...
           NULL, NULL, NULL,
           1, 0, NOW())`,
        { replacements: { rut: rutN, rutFmt, nombres, apPat: apellido_paterno, fecNac: fecha_nacimiento, genero, email }, transaction: t }
      );
    }

    // 4) Rol APODERADO
    const [[rol]] = await sequelize.query(
      `SELECT id, nombre_rol FROM roles WHERE UPPER(nombre_rol) = 'APODERADO' LIMIT 1`,
      { transaction: t }
    );
    if (rol?.id) {
      const [[ya]] = await sequelize.query(
        `SELECT 1 FROM persona_roles
         WHERE rut_persona = :rut AND rol_id = :rol LIMIT 1`,
    ...
    // token con rol apoderado
    const token = signFullToken({ rut: rutN, email: `${rutN}@auto.local`, roles: ['APODERADO'] });
...
router.post('/refresh', authGuard, async (req, res) => {
  try {
    // Reusar claims del token actual si existen
    const current = req.user || { sub: req.rut_persona };
    const token = signFullToken({
      rut: current.rut || current.sub,
      email: current.email || null,
      roles: current.roles || (current.role ? [current.role] : []),
    });
    return res.json({ success: true, data: { token } });
  } catch {
